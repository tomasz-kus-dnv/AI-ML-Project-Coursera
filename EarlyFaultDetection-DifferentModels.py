...
combined_df_downsampled = combined_df.copy() # downsample_every_nth_row(combined_df, n=6)
...
# === ADVANCED MODELS FOR IMPROVED ACCURACY ===
print(f"\n" + "="*60)
print("🚀 TRAINING ADVANCED MODELS FOR IMPROVED ACCURACY")
print(f"="*60)

print(f"\n📦 INSTALLATION GUIDE FOR BEST PERFORMANCE:")
print(f"   pip install xgboost lightgbm catboost")
print(f"   pip install optuna scikit-optimize  # for hyperparameter tuning")
...
from sklearn.svm import SVC
from sklearn.neural_network import MLPClassifier
# Advanced gradient boosting libraries (install if needed)
try:
...
def evaluate_model(model, X_train, y_train, X_test, y_test, model_name):
    ...
    # ROC-AUC and PR-AUC (important for imbalanced data)
    ...
    print(f"      ROC-AUC: {roc_auc:.4f} | PR-AUC: {pr_auc:.4f}" if roc_auc else "")
...
if XGBOOST_AVAILABLE:
    ...
    model_results['XGBoost'] = evaluate_model(xgb_model, X_train_smote, y_train_smote, X_test_scaled, y_test, "XGBoost")
...
if LIGHTGBM_AVAILABLE:
    ...
    model_results['LightGBM'] = evaluate_model(lgb_model, X_train_smote, y_train_smote, X_test_scaled, y_test, "LightGBM")
...
if CATBOOST_AVAILABLE:
    ...
    model_results['CatBoost'] = evaluate_model(catboost_model, X_train_smote, y_train_smote, X_test_scaled, y_test, "CatBoost")
...
model_results['GradientBoosting'] = evaluate_model(gb_model, X_train_smote, y_train_smote, X_test_scaled, y_test, "Gradient Boosting")
...
model_results['ExtraTrees'] = evaluate_model(et_model, X_train_smote, y_train_smote, X_test_scaled, y_test, "Extra Trees")
...
model_results['Bagging_RF'] = evaluate_model(bagging_rf, X_train_smote, y_train_smote, X_test_scaled, y_test, "Bagging + Random Forest")
...
model_results['MLP'] = evaluate_model(mlp_model, X_train_smote, y_train_smote, X_test_scaled, y_test, "Neural Network (MLP)")

# === 4. SUPPORT VECTOR MACHINES ===
print(f"\n🌟 4. SUPPORT VECTOR MACHINES")

# SVM with RBF kernel (smaller sample for speed)
svm_model = SVC(
    kernel='rbf',
    C=1.0,
    gamma='scale',
...
X_train_svm = X_train_smote[sample_idx]
y_train_svm = y_train_smote[sample_idx]
model_results['SVM'] = evaluate_model(svm_model, X_train_svm, y_train_svm, X_test_scaled, y_test, "SVM (RBF)")

# === 5. ADVANCED LOGISTIC REGRESSION ===
print(f"\n🌟 5. ADVANCED LOGISTIC REGRESSION")
...
model_results['LogReg_L1'] = evaluate_model(lr_l1, X_train_smote, y_train_smote, X_test_scaled, y_test, "Logistic Regression (L1)")
...
model_results['LogReg_L2'] = evaluate_model(lr_l2, X_train_smote, y_train_smote, X_test_scaled, y_test, "Logistic Regression (L2)")

# === 6. VOTING ENSEMBLE ===
print(f"\n🌟 6. VOTING ENSEMBLE")

# Create voting classifier with available models
voting_models = [
...
if XGBOOST_AVAILABLE:
    voting_models.append(('xgb', XGBClassifier(n_estimators=200, max_depth=6, random_state=42, eval_metric='logloss', use_label_encoder=False)))

if LIGHTGBM_AVAILABLE:
    voting_models.append(('lgb', LGBMClassifier(n_estimators=200, max_depth=6, random_state=42, verbose=-1)))
...
model_results['Voting_Soft'] = evaluate_model(voting_soft, X_train_smote, y_train_smote, X_test_scaled, y_test, "Voting Ensemble (Soft)")
...
model_results['Voting_Hard'] = evaluate_model(voting_hard, X_train_smote, y_train_smote, X_test_scaled, y_test, "Voting Ensemble (Hard)")
...
print(f"\n🎯 BEST MODELS FOR DIFFERENT OBJECTIVES:")
print(f"   🥇 Best F1-Score: {best_f1_model} ({comparison_df.iloc[0]['F1-Score']:.4f})")
print(f"   🎯 Best Recall (fault detection): {best_recall_model} ({comparison_df.loc[comparison_df['Recall'].idxmax(), 'Recall']:.4f})")
print(f"   ⚡ Best Precision (low false alarms): {best_precision_model} ({comparison_df.loc[comparison_df['Precision'].idxmax(), 'Precision']:.4f})")
print(f"   📈 Best ROC-AUC: {best_roc_auc_model} ({comparison_df.loc[comparison_df['ROC-AUC'].idxmax(), 'ROC-AUC']:.4f})")

# === SAVE BEST MODEL ===
best_model_name = best_f1_model
best_model_result = model_results[best_model_name.replace(' ', '').replace('(', '_').replace(')', '_').replace('-', '_')]
best_model = best_model_result['model']

print(f"\n💾 BEST MODEL SELECTED: {best_model_name}")
print(f"   Performance Summary:")
print(f"   • F1-Score: {best_model_result['f1_score']:.4f}")
print(f"   • Recall: {best_model_result['recall']:.4f} (catches {best_model_result['tp']}/{(y_test==1).sum()} faults)")
print(f"   • Precision: {best_model_result['precision']:.4f}")
print(f"   • False Alarms: {best_model_result['fp']}/{(y_test==0).sum()}")
print(f"   • ROC-AUC: {best_model_result['roc_auc']:.4f}")

# Update the global model variable to use the best model
model = best_model

print(f"\n✅ Model variable updated to use {best_model_name}")
print(f"🔄 You can now use this improved model with predict_with_threshold() function")
...
if comparison_df.iloc[0]['F1-Score'] > 0.8:
    print(f"   ✅ Excellent performance! Consider deployment.")
elif comparison_df.iloc[0]['F1-Score'] > 0.6:
    print(f"   ⚡ Good performance. Consider hyperparameter tuning.")
else:
    print(f"   ⚠️  Performance needs improvement. Try feature engineering or more data.")

print(f"\n🔧 NEXT STEPS TO TRY:")
print(f"   1. Hyperparameter tuning with GridSearchCV or Optuna")
print(f"   2. Feature selection to remove noise")
print(f"   3. Time-series aware cross-validation")
print(f"   4. Anomaly detection methods (Isolation Forest, One-Class SVM)")
print(f"   5. Deep learning models (LSTM for time series)")
print(f"   6. Stacking ensembles (train meta-learner on predictions)")

print(f"\n🎯 PRODUCTION DEPLOYMENT:")
print(f"   • Use {best_model_name} as your main model")
print(f"   • Implement threshold optimization for your specific use case")
print(f"   • Set up model monitoring and retraining pipeline")
print(f"   • Consider ensemble of top 3 models for critical applications")
